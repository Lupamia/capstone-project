...
const BarangayOfficials = ({ title }) => {
  ...
  const handleApprove = async (officialId) => {
    ...
    try {
      await axios.put(
        `${backendUrl}/api/officials/${officialId}/approve-barangay`,
        null,
        { withCredentials: true }
      );
      setSuccessMessage("Barangay official approved successfully!");
      fetchBarangays();

      // 👇 Close modal after success
      closeFormModal();
    } catch (err) {
      console.error("Failed to approve barangay official:", err);
      setError(
